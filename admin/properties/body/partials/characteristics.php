<div data-modal="characteristics" class="z-[99999] fixed top-0 left-0 w-full h-full items-center justify-center hidden z-50">
    <div class="bg-white rounded w-full max-w-[500px]" data-modal-body="popup">
        <div class="p-2 relative bg-color-main rounded-t">
            ...
            <h2 class="font-bold text-white p-2 rounded text-center" id="modalDeleteItemLabel"><?php _e('Add characteristics') ?></h2>
        </div>

        <div class="p-4">
            <div>
                <div class="h-[450px] overflow-x-auto" data-list="characteristics">
                    <?php if (is_array($characteristics)) {
                        foreach ($characteristics as $indice => $characteristic) { ?>
                            <div class="my-3 flex w-full">
                                ...
                                <button class="bg-danger rounded-r text-white px-4 font-bold border border-danger hover:bg-white hover:text-danger ease-in duration-300" type="button" title="<?php _e('Characteristic :indice', [':indice' => $indice + 1]) ?>" data-item="remove">
