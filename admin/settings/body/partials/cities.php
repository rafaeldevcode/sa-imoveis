<div data-modal="cities" class="z-[99999] fixed top-0 left-0 w-full h-full items-center justify-center hidden z-50">
    <div class="bg-white rounded w-full max-w-[500px]" data-modal-body="popup">
        <div class="p-2 relative bg-color-main rounded-t">
            ...
            <h2 class="font-bold text-white p-2 rounded text-center" id="modalDeleteItemLabel"><?php _e('Add cities') ?></h2>
        </div>

        <div class="p-4">
            <div>
                <div class="h-[450px] overflow-x-auto" data-list="cities">
                        ...
                        foreach ($cities as $indice => $city) { ?>
                            <div class="my-3 flex w-full">
                                <label class="relative block w-full">
                                    ...
                                    <input value="<?php echo $city ?>" class="placeholder:italic placeholder:text-secondary block bg-white w-full border border-secondary rounded-l py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-color-main focus:ring-color-main focus:ring-1 sm:text-sm" placeholder="<?php _e('Cities') ?> <?php $indice + 1 ?>" type="text" name="cities[]" onkeyup="ChangeLocationMaps.init(event)">
                                </label>
                                <button class="bg-danger rounded-r text-white px-4 font-bold border border-danger hover:bg-white hover:text-danger ease-in duration-300" type="button" title="<?php _e('City :indice', [':indice' => $indice + 1]) ?>" data-item="remove">
                ...
                <button id="add-cities" type="button" title="<?php _e('Add') ?>" class="mx-auto btn btn-color-main font-bold mt-4">
                    <?php _e('Add') ?>
                </button>
            </div>
